<template>
  <div
    :class="['search__outer', {'light': light}, {'disabled': disabled}, {'invalid': invalid}]">
    <!-- Label -->
    <label
      for="name"
      :class="['search__label', {'dirty': dirty}]"
      @click="toggleOpen()">
      {{ label }}
    </label>
    <!-- inner -->
...
export default {
  ...
  computed: {
    ...
    searchResults() {
      const input = this.$attrs.value.toLowerCase() || null;
      if (!input || this.select) return this.options;
  ...
  methods: {
    ...
    handleFocus(value) {
      this.focus = true;
      // this.$scrollTo("#submissionForm", 300, {
      //   offset: 9999,
      // });
    ...
    evaluateOpen(value) {
      ...
      if (this.searchResults.length > 1 || value !== this.searchResults[0] || this.select) {
        this.setOpen(true);
      } else {
        this.setOpen(false);
      }
